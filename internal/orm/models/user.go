...
type User struct {
	...
	//nolint (required because of lll long lines)
...
type UserProfile struct {
	...
	Provider       string    `gorm:"not null;index;unique_index:idx_email_provider_external_user_id;default:'DB'"`
...
func (k *UserAPIKey) BeforeSave(scope *gorm.Scope) error {
	...
	if hash, err := bcrypt.GenerateFromPassword([]byte(k.UserID.String()), 0); err == nil {
		...
		_ = scope.SetColumn("APIKey", hex.EncodeToString(hasher.Sum(nil)))
	}
